  ...
  try {
    ...
    if (code) {
      const result = await pool
        ...
        .query(`
          SELECT Employee_Code AS emp_code, Employee_Name AS emp_name, Grade_Name AS designation, Department_Name AS department, Funcational_Area AS functional_area FROM Employee_DB.dbo.Employee_Master WHERE Employee_Code = @code AND Status = 1
        `);
      ...
      return NextResponse.json(result.recordset[0]); // ✅ SINGLE OBJECT
    }
